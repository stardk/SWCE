  ...
  .app-container
    transition(name="fade")
      .popup(v-if="showPopup")
        ...
        CharacterPopup(v-else :character="chosenCharacter" @close-popup="closePopup")
        .popup__overlay(@click="closePopup")
    .content(v-infinite-scroll="loadNextCharacters" infinite-scroll-disabled="loading" infinite-scroll-distance="10" :class="{'underPopup': showPopup}")
      header
        .logo
          .logo__star-wrapper
      ...
      main
        transition(name="fade" mode="out-in")
          .search(v-if="characters.length > 0")
            .search__input 
        ...
        transition(name="fade" mode="out-in")
          Loading.main-loading(v-if="!showPopup && loading" key="fetching")
          ...
          .main-content(v-else key="fetched")
            ...
            CharacterCard.card-list-item(
              v-if="searchResult.length > 0"
              v-for="(character, index) in searchResult"
              :key="index"  
              :character="character"
              @click.native="onCardClick(character.url)"
            )
      footer(v-if="!loading") STAR WARS CHARACTER Encyclopedia, 2019
</template>
  ...
  methods: {
    ...
    async search() {
      this.$store.dispatch('searchCharacters', { search: this.searchValue })
        .then(() => {
          this.searchResult = this.$store.getters.getCharacters;
          this.loading = false;
        })
    },
  ...
  .app-container {
    ...
    .content {
      ...
      main {
        ...
        .search {
          width: 100%;
          margin: calc(100vh / 12) 14% 64px 14%;
